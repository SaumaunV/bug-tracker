...
function DashboardInfo() {
  ...
  const userTickets = useQuery(GET_USER_ALL_TICKETS, {
    variables: { id: user ? user.id : "" },
  });
  ...
  const completedTickets = tickets.data?.userTickets?.filter(
    (ticket) => ticket.status === "resolved"
  ).length;

  if (loading || tickets.loading || users.loading || userTickets.loading)
    return <Skeleton m={8} h="150px"></Skeleton>;
  else
    return (
